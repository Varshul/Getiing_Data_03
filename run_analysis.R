...
runAnalysis <- function() {
  ...
  tidy <- ddply(tidy, .(Subject, Activity), limitedColMeans)
  names(tidy)[-c(1,2)] <- paste0("Mean", names(tidy)[-c(1,2)])
  
  # Write file
  write.table(tidy, "tidy_set.txt", row.names = FALSE)
  
  tidy_set
}

# Use to check that the tidy_set.txt is properly readable
checkData <- function() {
  read.table("tidy_set.txt", header = TRUE)
}